...
module Make
	...
	let atomic_pair_allowed e1 e2 =
		match e1.E.iiid, e2.E.iiid with
	...
	module Mixed(SZ : ByteSize.S) = struct
      ...
      let rec build_semantics_expr is_data e ii : V.v M.t =
        ...
        | JavaBase.LoadMem (vh, am) -> (read_reg is_data vh ii) >>=
                              (fun l -> read_mem is_data (AM am) l ii)
        | JavaBase.Const i ->
              (M.unitT (V.maybevToV (Constant.Concrete
                 (ParsedConstant.StringScalar.of_int i))))
        ...
        | JavaBase.Rmw (vh, (op, am), e) ->
              ...
              (fun (l , v) -> fetch_op op v (MOOrAN.AM am) l ii)
        | JavaBase.CAS (vh, (read_am, write_am), expect, dest) ->
              ...
              (read_mem true (MemOrderOrAnnot.AM read_am) loc_vh ii) >>*= fun v_vh ->
              M.altT
                ((M.neqT v_vh v_expect) >>! v_vh)
                ((build_semantics_expr true dest ii) >>= fun v_dest ->
                    (M.mk_singleton_es
                    (Act.RMW (A.Location_global loc_vh,
                      v_expect, v_dest, (MemOrderOrAnnot.AM
                            (match read_am , write_am with
                              | AccessModes.Acquire, AccessModes.Plain ->
                                  read_am
                              | AccessModes.Plain, AccessModes.Release ->
                                  write_am
                              | _ -> write_am)), nat_sz))
                    ii) >>! v_expect)
      ...
      let rec build_semantics ii : (A.program_order_index * B.t) M.t =
        let ii =
            ...
            | JavaBase.StoreMem (vh, am, e) -> (
                    ...
                    (build_semantics_expr true e ii) >>=
                    (fun (l, v) -> write_mem (AM am) l v ii) >>=
                    (fun _ -> M.unitT (ii.A.program_order_index, B.Next)))
